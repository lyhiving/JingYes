...
function clearTextNode(node, option) {
  ...
  var stop = false,
    ...
    ,
    skipChild = option.skipChild || 'skipChild' //遇到class跳过内部
    ,
    skipNext = option.skipNext || 'skipNext' //遇到class跳过紧跟text节点
    ,
    stopClass = option.stopClass || 'stop'; //遇到class终止
  node.normalize();
  walk(node);
  node.normalize();
  ...
  function skip(node) {
    ...
    return node.classList.contains(skipChild);
  }
  ...
  function walk(node) {
    ...
    while (!stop && node) {
      ...
      if (node.nodeType==1 && node.classList.contains(skipNext)){
...
$('[hidden]').on('click.jingyes.collapse',function(){
